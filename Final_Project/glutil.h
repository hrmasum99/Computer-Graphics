...
void typedText(float x, float y, char* strings) {
    ...
    char* token = strtok(currentText, "\n"); // Split text into lines using newline as delimiter
    
    while (token != NULL) {
        _drawString(x, y, token); // Draw each line separately
        y -= 0.5; // Adjust Y position for next line
        token = strtok(NULL, "\n");
    }
}
